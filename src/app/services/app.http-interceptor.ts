...
export class AppHttpInterceptor implements HttpInterceptor {
	...
	constructor(private _snackBar: SnackbarComponent,private cacheRegistrationService: CacheRegistrationService) {
	...
	public intercept(
		...
		return handler.handle(httpRequest).pipe(
			tap(
				...
				err => {
					this._snackBar.openSnackBar('Something bad happened',"Please try again later!!");
				}
			)
		);
		...
		if (lastResponse) {
			...
			return lastResponse instanceof Observable
				? lastResponse
				: Observable.of(lastResponse.clone());
		}
